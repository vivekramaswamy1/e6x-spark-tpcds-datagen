...
import org.apache.spark.sql.execution.benchmark.TPCDSDatagen.TableMultiplicationFactorMap
...
class Tables(sqlContext: SQLContext, scaleFactor: Int) extends Serializable {
  ...
  private case class Table(name: String, partitionColumns: Seq[String], schema: StructType) {
     ...
     */
    def df(convertToSchema: Boolean, numPartition: Int, numberOfParallelProcesses: Int): DataFrame = {
      val partitions = if (partitionColumns.isEmpty) numberOfParallelProcesses else numPartition
      val generatedData = {
    ...
    def genData(
        ...
        numPartitions: Int,
        fileSizeInMB: Int,
        numberOfParallelProcess:Int): Unit = {
      val mode = if (overwrite) SaveMode.Overwrite else SaveMode.Ignore

      val numberOfParttitionFiles = getNumPartitions(name, fileSizeInMB)

      val data = df(format != "text", numPartitions, numberOfParallelProcess)
      ...
      val writer = if (partitionColumns.nonEmpty) {
        ...
        data.repartition(numberOfParttitionFiles).write
      }
  ...
  def genData(
      ...
      fileSizeInBytes: Int = 128,
      numberOfParallelProcess: Int = 1): Unit = {
    ...
    withSpecifiedDataType.foreach { table =>
      ...
      table.genData(tableLocation, format, overwrite, clusterByPartitionColumns,
        filterOutNullPartitionValues, numPartitions, fileSizeInBytes, numberOfParallelProcess)
    }
  }
  ...
  def getNumPartitions(tableName: String, fileSizeInMB: Int) : Int = {
    val multiplicationFactor = TableMultiplicationFactorMap.get(tableName) match {
      case x: Some[Double] => x.get
      case _ => 1.toDouble
    }

    val numPartitions =  java.lang.Math.ceil(
      (multiplicationFactor * scaleFactor  ) / fileSizeInMB).toInt
...
object TPCDSDatagen {
  ...
  def TableMultiplicationFactorMap: Map[String, Double] = Map(
    "item" -> 1.086914063,
    "catalog_page" -> 0.169921875,
    "customer" -> 3.103515625,
    "web_page" -> 0.004882813,
    "promotion" -> 0.009765625,
    "reason" -> 0.00390625,
    "call_center" -> 0.005859375,
    "date_dim" -> 0.452148438,
    "time_dim" -> 0.28515625,
    "web_sales" -> 68.47949219,
    "household_demographics" -> 0.010742188,
    "customer_address" -> 0.5703125,
    "customer_demographics" -> 1.883789063,
    "store_sales" -> 181.3847656,
    "inventory" -> 57.7734375,
    "warehouse" -> 0.004882813,
    "income_band" -> 0.00390625,
    "store_returns" -> 16.14257813,
    "web_returns" -> 5.049804688,
    "web_site" -> 0.006835938,
    "store" -> 0.006835938,
    "catalog_returns" -> 10.15722656,
    "catalog_sales" -> 137.5429688,
    "ship_mode" -> 0.00390625
  )
  ...
  def main(args: Array[String]): Unit = {
    ...
    tpcdsTables.genData(
      ...
      datagenArgs.fileSizeInMB.toInt,
      datagenArgs.numberOfParallelProcess.toInt
    )
    spark.stop()
  }
}
