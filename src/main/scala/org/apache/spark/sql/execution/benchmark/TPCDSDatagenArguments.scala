...
class TPCDSDatagenArguments(val args: Array[String]) {
  ...
  var fileSizeInMB = "128"
  var numberOfParallelProcess = "1"
  ...
  private def parseArgs(inputArgs: List[String]): Unit = {
    ...
    while(args.nonEmpty) {
      args match {
        ...
        case ("--file-size") :: value :: tail =>
          fileSizeInMB = value
          args = tail

        case ("--num-process") :: value :: tail =>
          numberOfParallelProcess = value
          args = tail
  ...
  private def printUsageAndExit(exitCode: Int): Unit = {
    ...
    System.err.println("""
      ...
      |  --file-size [NUM]                      size of output files in MB (default: 128)
      |  --num-process [NUM]                    number of parallel processes (default: 1)
      |
      """.stripMargin)
    // scalastyle:on
  ...
  private def validateArguments(): Unit = {
    ...
    if (Try(fileSizeInMB.toInt).getOrElse(-1) <= 0) {
      // scalastyle:off println
      System.err.println("file size must be a positive number")
      // scalastyle:on println
      printUsageAndExit(-1)
    }
    if (Try(numberOfParallelProcess.toInt).getOrElse(-1) <= 0) {
      // scalastyle:off println
      System.err.println("number Of Parallel Process must be a positive number")
      // scalastyle:on println
      printUsageAndExit(-1)
    }
  }
